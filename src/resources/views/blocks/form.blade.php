<section class="form-section py-16">
    <div class="container mx-auto px-4 max-w-2xl">
        @if(!empty($title))
            <h2 class="text-3xl font-bold text-center mb-4">{{ __($title) }}</h2>
        @endif
        
        @if(!empty($description))
            <p class="text-center mb-8">{{ __($description) }}</p>
        @endif
        ...
        <form action="{{ route('pagebuilder.form.submit') }}" method="POST" class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="mb-4">
                <label for="name" class="block text-sm font-medium mb-1">{{ __('pagebuilder::messages.name') }}</label>
                <input type="text" name="name" id="name" required class="w-full p-2 border rounded">
            </div>
            
            <div class="mb-4">
                <label for="email" class="block text-sm font-medium mb-1">{{ __('pagebuilder::messages.email') }}</label>
                <input type="email" name="email" id="email" required class="w-full p-2 border rounded">
            </div>
            
            <div class="mb-4">
                <label for="message" class="block text-sm font-medium mb-1">{{ __('pagebuilder::messages.message') }}</label>
